  ...
  try {
    ...
    try {
      ...
      await Notification.create({
        ...
        senderName: 'Admin',
        title: `Leave ${status}`,
        message: `Your ${leave.leaveType || ''} leave request (${new Date(leave.startDate).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })} - ${new Date(leave.endDate).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })}) has been ${status.toLowerCase()}${summaryText}.${remarks ? ` Note: ${remarks}` : ''}`,
  ...
  try {
    ...
    try {
      await Notification.create({
        ...
        message: `Admin updated your ${leave.leaveType} leave request (${new Date(leave.startDate).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })} - ${new Date(leave.endDate).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })}) status to ${leave.status}.${leave.adminRemarks ? ` Note: ${leave.adminRemarks}` : ''}`,
